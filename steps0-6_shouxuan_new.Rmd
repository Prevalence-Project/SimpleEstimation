...
step1_results <- results_data4 %>% 
  ...
  mutate(est_num_untreated_oud = (od_deaths/lambda) - (0.25 * medi_bup)) %>% 
  mutate(est_num_untreated_oud = ifelse(est_num_untreated_oud>Population,Population,est_num_untreated_oud)) %>%
  mutate(est_num_untreated_oud = ifelse(est_num_untreated_oud<0,0,est_num_untreated_oud)) %>%
  filter(Year != "2020") |> 
...
#saveRDS(step5_results, "Results/step0-5results_new.rds")
```
...
saveRDS(step6_results, "Results/step6results_new.rds")
```
